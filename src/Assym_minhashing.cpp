...
void read_array()
{
    std::ifstream input("/home/trahman/GitRepo/pakman-pakman_latest/Prime.txt");

    for (int i = 0; i < 200; i++) {
        input >> Px[i];
        //std::cout<< A[i]<<std::endl;
    }
    std::ifstream input1("/home/trahman/GitRepo/pakman-pakman_latest/A.txt");

    for (int i = 0; i < 200; i++) {
        input1 >> Ax[i];
        //std::cout<< A[i]<<std::endl;
    }
    std::ifstream input2("/home/trahman/GitRepo/pakman-pakman_latest/B.txt");
...
*/
void recalculate_min_lmer (std::string ptr, lmer_t *m_kmer, int *fact, int *pos)
{
    lmer_t min_lmer=3074457345618258602, tmp_lmer=0;
    //lmer_t min_lmer_freq=0, tmp_lmer_freq=0;
    //int min_pos=0, k=0;
    ...
    for(int i=0; i<16-1; i++) {
      ...
      //kmer = (kmer_t)((kmer<<2) | (kmer_t)(convert_to_int(ptr[p]))) & (kmer_t)KMER_MASK;
        kmer = lmer_shift(kmer, char_to_el(ptr[i]));
            /*
            if(rank == 0)
            {
                std::cout<<ptr[p]<<"\n";
                     ...
                     std::vector<std::unordered_map<kmer_t, std::vector<int>> > Tl, int start, int total_subjects)
{
    ...
    kmer_t kmer = 0; 
    lmer_t min_kmer = 0;
    lmer_t rev_min_kmer = 0;
    kmer_t rev_kmer = 0; 
    lmer_t min_lmer_freq = 0; 
    int min_pos = 0; 
    int m_pos = 0;
    int min_val;
    int val;
    ...
    while(p<length) {
      ...
      //kmer = (kmer_t)((kmer<<2) | (kmer_t)(convert_to_int(ptr[p]))) & (kmer_t)KMER_MASK;
            ...
            p++;
            contig_len++;
            recalculate_min_lmer(str.substr(contig_len - 100, 100), &min_kmer, &min_pos, &m_pos);
      ...
      //kmer = (kmer_t)((kmer<<2) | (kmer_t)(convert_to_int(ptr[p]))) & (kmer_t)KMER_MASK;
        ...
        while(tracker<contig_len) {
            //rev_kmer = kmer_shift(rev_kmer, char_to_el(s[tracker]));
            tracker++;
            recalculate_min_lmer(s.substr(tracker - 100, 100), &rev_min_kmer, &min_pos, &m_pos);
                     ...
                     std::vector<MinHashPairs> &initial_sets, int s_index)
{
    ...
    while(p<length) {
      ...
      //kmer = (kmer_t)((kmer<<2) | (kmer_t)(convert_to_int(ptr[p]))) & (kmer_t)KMER_MASK;
            ...
            contig_len++;
            //rev_set.push_back(min_kmer);
            recalculate_min_lmer(str.substr(contig_len - 100, 100), &min_kmer, &min_pos, &ex_pos);
            rev_set.push_back(min_kmer);
      ...
      //kmer = (kmer_t)((kmer<<2) | (kmer_t)(convert_to_int(ptr[p]))) & (kmer_t)KMER_MASK;
        ...
        while(tracker<contig_len) {
            ...
            recalculate_min_lmer(s.substr(tracker - 100, 100), &rev_min_kmer, &min_pos, &ex_pos);
